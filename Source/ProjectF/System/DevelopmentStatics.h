...
#include "DevelopmentStatics.generated.h"
...
UCLASS()
class UDevelopmentStatics : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()

public:
	// Should game logic skip directly to gameplay (skipping any match warmup / waiting for players / etc... aspects)
	// Will always return false except when playing in the editor and bTestFullGameFlowInPIE (in  Developer Settings) is false
	UFUNCTION(BlueprintCallable, Category="ProjectF")
	static bool ShouldSkipDirectlyToGameplay();

	// Should game logic load cosmetic backgrounds in the editor?
	// Will always return true except when playing in the editor and bSkipLoadingCosmeticBackgroundsInPIE (in  Developer Settings) is true
	UFUNCTION(BlueprintCallable, Category = "ProjectF", meta=(ExpandBoolAsExecs="ReturnValue"))
